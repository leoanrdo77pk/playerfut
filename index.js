...
module.exports = async (req, res) => {
  ...
  https.get(targetUrl, {
    ...
    resp.on('end', () => {
      // Ajusta links para o domínio Vercel
      data = data
      ...
      // Código do banner fixo + popup para injetar
      const bannerAndPopup = `
<script>
// === BANNER FIXO ===
...
(function () {
  ...
  window.addEventListener("DOMContentLoaded", schedulePopup);
      ...
      // Injeta o banner e popup antes do fechamento da tag </body>
      const finalHtml = data.includes('</body>') ? data.replace('</body>', bannerAndPopup) : data + bannerAndPopup;
